# ~/Projects/datum-config/backups.nix
{
  ...
  backupStatus = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # NOT `snapshots latest` -- "latest" there is a positional snapshot ID
      # and resolves to a single snapshot repo-wide (the most recently
      # *created* one), not one per group. Guest always backs up a few
      # seconds after rik in the same run, so that silently hid the rik
      # snapshot every single time: guest's slightly newer timestamp always
      # "won" and rik's never appeared.
      #
      # --filter-last 1 is the actual "newest per group" query; grouping
      # defaults to host,label,paths, so with --as-path=/home/rik and
      # /home/guest that is one row per profile.
      if ! ${lib.getExe backupRustic} snapshots --filter-last 1; then
        echo "  (could not list latest snapshots; showing everything)"
...
in {
  ...
  systemd.services.rustic-atomic-backup = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "rustic-atomic-runner" ''
        ...
        #
        # `|| true` is deliberate: btrbk-local (hourly) and this service
        # (daily) both fire at :00:00, so at midnight they race. If btrbk is
        # mid-prune when find stats a subvolume it's in the middle of
        # deleting, find reports that one entry as gone and exits 1 -- even
        # though it already listed every other entry fine, including
        # whatever btrbk just created. Under pipefail that nonzero status
        # would otherwise trip `set -e` right here and abort before the
        # empty-result check below ever runs, discarding a perfectly good
        # $LATEST_SNAP. (Diagnosed from the 2026-09-01T00:00 failure: find
        # choked on a vanishing @home.20260829T1000 while btrbk was pruning
        # 23 stale hourly snapshots, one second after creating
        # @home.20260901T0000 -- the very snapshot this should have used.)
        LATEST_SNAP=$(find /mnt/btrfs-root/.snapshots/home/ -maxdepth 1 -type d -name "@home.*" | sort -r | head -n 1) || true
  ...
  systemd.timers.rustic-atomic-backup = {
    ...
    timerConfig = {
      # NOT "daily" (= *-*-* 00:00:00) -- btrbk-local is "hourly" (=
      # *:00:00), so midnight is also one of its firings, and the two used
      # to land in the same second. When btrbk's prune deleted an aged-out
      # snapshot while this service's `find` was mid-stat on it, find's
      # transient nonzero exit (see LATEST_SNAP above) killed the whole
      # backup -- see the 2026-09-01T00:00 failure. Offsetting 15 minutes
      # past the hour keeps this "nightly, around midnight" while clearing
      # every hourly boundary btrbk can fire on, not just this one.
      OnCalendar = "*-*-* 00:15:00";
      Persistent = true;
    };
  };
